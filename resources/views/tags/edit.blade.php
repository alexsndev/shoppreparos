...
<div class="container mx-auto py-8 max-w-md">
    ...
    <form action="{{ route('tags.update', $tag) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" class="w-full border rounded px-3 py-2" value="{{ old('name', $tag->name) }}" required>
            @error('name')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1 font-semibold">Slug</label>
            <input type="text" name="slug" class="w-full border rounded px-3 py-2" value="{{ old('slug', $tag->slug) }}" required>
